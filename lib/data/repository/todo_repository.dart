...
class TodoRepository {
  ...
  Future<Map<String, dynamic>> getTodos(String fromDate, String toDate) async {
    Map<String, dynamic> response = await remoteDataSource.getTodos(fromDate, toDate);
  ...
  Future<Map<String, dynamic>> postTodo(Todo todo, String fromDate, String toDate) async {
    if (todo.title == "" || todo.repeat == null) {
      return {Strings.message: Strings.noValue};
    }
    Map<String, dynamic> response = await remoteDataSource.postTodo(todo, fromDate, toDate);
    if (response[Strings.result]) {
  ...
  Future<Map<String, dynamic>> doneTodo(Todo todo, bool isDone, String fromDate, String toDate) async {
    Map<String, dynamic> response = await remoteDataSource.doneTodo(todo, isDone ? 1 : 0, fromDate, toDate);
    if (response[Strings.result]) {
      Map<String, dynamic> data = response[Strings.response].data["data"]["updateLists"];
      return {
        Strings.message: Strings.success,
  ...
  Future<Map<String, dynamic>> updateTodo(Todo todo, bool isAfterUpdate, String fromDate, String toDate) async {
    Map<String, dynamic> response = await remoteDataSource.updateTodo(todo, isAfterUpdate ? 1 : 0, fromDate, toDate);
